...
async fn submit(content: web::Form<FormData>, data: web::Data<AppState>) -> impl Responder {
    ...
    let redirect_url = format!("/paste/{}", token);

    HttpResponse::SeeOther().append_header((header::LOCATION, redirect_url)).finish()
}
async fn get_paste(token: web::Path<String>, data: web::Data<AppState>) -> impl Responder {
    let conn = data.db.lock().unwrap();
    let content = conn
        .query_row("SELECT content FROM pastes WHERE token = ?", params![token.to_string()], |row|
            row.get::<_, String>(0)
        )
...
async fn main() -> std::io::Result<()> {
    let db = Connection::open("paste.db").expect("Failed to open database");
    db.execute(
